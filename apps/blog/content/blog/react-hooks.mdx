...
title: React Hooks
date: 2022-10-21
description: Syntax of native hooks and useful custom hooks
...
function LargeList({ term, onItemClick }) {
  const items = useSearch(term);
  const map = (item) => <div onClick={onItemClick}>{item}</div>;

  return <div>{items.map(map)}</div>;
}
export const MemoizedLargeList = React.memo(LargeList);
```

```tsx title="useCallback"
